...
func main() {
	...
	// if (*scriptPath != "") && (!*clusterDiscovery) {
	...
	// }
	promConf.Collector = exp
	promConf.Registry = prometheus.NewRegistry()
	err = promConf.Registry.Register(promConf.Collector)
	if err != nil {
		log.Errorln("Init Prom: Couldn't register collector:", err)
		return
	}

	promConf.Gatherers = &prometheus.Gatherers{
		prometheus.DefaultGatherer,
		promConf.Registry,
	}

	http.HandleFunc(*metricPath, handlerCluster)
...
func handlerCluster(w http.ResponseWriter, r *http.Request) {
	h := promhttp.InstrumentMetricHandler(
		promConf.Registry,
		promhttp.HandlerFor(promConf.Gatherers,
